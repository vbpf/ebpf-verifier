...
static __always_inline int handle_ipv4(struct xdp_md *xdp, void* data, void* data_end)
{
...
static __always_inline int handle_ipv6(struct xdp_md *xdp, void* data, void* data_end)
{
	struct iptnl_info *tnl;
	struct ethhdr *new_eth;
	struct ethhdr *old_eth;
...
int _xdp_tx_iptunnel(struct xdp_md *xdp)
{
	...
	if (h_proto == htons(ETH_P_IP))
		return handle_ipv4(xdp, data, data_end);
	else if (h_proto == htons(ETH_P_IPV6))

		return handle_ipv6(xdp, data, data_end);
	else
		return XDP_PASS;
}
