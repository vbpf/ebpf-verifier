...
void
abs_execute_assume(struct abs_state *state, struct ebpf_inst inst, bool taken)
{
...
void
abs_join_all(struct abs_state *state, struct abs_state *more_states, uint32_t num_more_states)
{
    for (int i=0; i < num_more_states; i++) {
        for (int r = 1; r < 11; i++) {
            if (!more_states[i].known[r] || state->reg[r] != more_states[i].known[r])
                state->known[r] = false;
        }
    }
}
...
void
abs_execute(struct abs_state *state, struct ebpf_inst inst)
{
