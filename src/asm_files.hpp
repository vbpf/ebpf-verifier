...
std::vector<raw_program_t> read_raw(std::string path, program_info info);
std::vector<raw_program_t> read_elf(const std::string& path, const std::string& desired_section,
                                    const ebpf_verifier_options_t& options, const ebpf_platform_t* platform);
std::vector<raw_program_t> read_elf(std::istream& input_stream, const std::string& path,
