...
RCP_domain MemDom::load(const OffsetDomSet &offset_dom, uint64_t _width) const {
    ...
    if (total_width == 0)
        return {TOP};
    if (min_offset > offset || max_end < offset + width)
        return {TOP};
    if (!all_must_be_num)
        return {TOP};
    if (min_offset + total_width < max_end)
        return {TOP};
    return numtop();
}
...
void MemDom::store_dynamic(const OffsetDomSet &offset_dom, const NumDomSet &_width, const RCP_domain &value) {
    ...
    int64_t max_offset =
        offset_dom.is_top() ? STACK_SIZE : *std::max_element(offset_dom.elems.begin(), offset_dom.elems.end());
    uint64_t min_width = _width.is_top() ? 0 : *std::min_element(_width.elems.begin(), _width.elems.end());
    uint64_t max_width =
        _width.is_top() ? STACK_SIZE - min_offset : *std::max_element(_width.elems.begin(), _width.elems.end());
...
void MemDom::store(const OffsetDomSet &offset_dom, uint64_t _width, const RCP_domain &value) {
    ...
    for (auto p : to_remove)
        cells.erase(std::remove(cells.begin(), cells.end(), p), cells.end());
    for (auto p : pieces)
        cells.push_back(p);
