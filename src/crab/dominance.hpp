...
void dominator_tree(G g, typename G::node_t entry, Map &idom) {
    ...
    std::vector<node_t> dom_tree_pred_vector(num_vertices(g), boost::graph_traits<G>::null_vertex());
    pred_map_t dom_tree_pred_map(make_iterator_property_map(dom_tree_pred_vector.begin(), index_map));

    boost::lengauer_tarjan_dominator_tree(g, entry, index_map, df_num_map, parent_map, vertices_by_df_num,
                                          dom_tree_pred_map);
    ...
    for (auto v : boost::make_iterator_range(vertices(g))) {
        if (get(dom_tree_pred_map, v) != boost::graph_traits<G>::null_vertex()) {
            ...
            CRAB_LOG("dominator", crab::outs() << crab::cfg_impl::get_label_str(get(dom_tree_pred_map, v))
                                               << " is the immediate dominator of " << crab::cfg_impl::get_label_str(v)
                                               << "\n";);

        } else {
            idom.insert(typename Map::value_type(v, boost::graph_traits<G>::null_vertex()));
            CRAB_LOG("dominator", crab::outs() << crab::cfg_impl::get_label_str(v) << " is not dominated by anyone!\n");
        }
    }
#endif
}
...
void dominance(G g, typename G::node_t entry, VectorMap &df) {
    ...
    for (auto n : boost::make_iterator_range(vertices(g))) {
        for (auto e : boost::make_iterator_range(in_edges(n, g))) {
            ...
            while (runner != boost::graph_traits<G>::null_vertex() && runner != idom[n] && runner != n) {
