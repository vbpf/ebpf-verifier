...
class interval_t final {
  ...
  public:
    interval_t(const bound_t& lb, const bound_t& ub) : _lb(lb), _ub(ub) {
        if (lb > ub) {
            _lb = bound_t{0};
            _ub = bound_t{-1};
        }
    ...
    explicit interval_t(const bound_t& b) : _lb(b), _ub(b) {
        if (b.is_infinite()) {
            _lb = bound_t{0};
            _ub = bound_t{-1};
        }
    ...
    explicit interval_t(const std::string& b) : _lb(b), _ub(b) {
        if (_lb.is_infinite()) {
            _lb = bound_t{0};
            _ub = bound_t{-1};
        }
