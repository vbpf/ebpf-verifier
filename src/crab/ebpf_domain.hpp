...
class array_domain_t final {
  ...
  public:
    ...
    std::optional<linear_expression_t> load(const NumAbsDomain& inv, data_kind_t kind, const linear_expression_t& i, int width) {
        interval_t ii = inv.eval_interval(i);
        if (std::optional<number_t> n = ii.singleton()) {
            offset_map_t& offset_map = lookup_array_map(kind);
            long k = (long)*n;
    ...
    void store_numbers(NumAbsDomain& inv, variable_t _idx, variable_t _width) {
        ...
        std::optional<number_t> idx_n = inv.eval_interval(_idx).singleton();
        if (!idx_n) {
            CRAB_WARN("array expansion store range ignored because ", "lower bound is not constant");
            return;
        }

        std::optional<number_t> width = inv.eval_interval(_width).singleton();
        if (!width) {
...
class ebpf_domain_t final {
  ...
  public:
    ...
    void do_store_stack(NumAbsDomain& inv, int width, A addr, X val_type, Y val_value,
        ...
        stack.store(inv, data_kind_t::types, addr, width, val_type);
        if (width == 8) {
            stack.store(inv, data_kind_t::values, addr, width, val_value);
            if (opt_val_offset && get_type(val_type) != T_NUM)
                stack.store(inv, data_kind_t::offsets, addr, width, *opt_val_offset);
            else
