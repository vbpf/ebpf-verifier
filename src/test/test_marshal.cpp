...
constexpr ebpf_inst EXIT_INST{.opcode = INST_OP_EXIT};
...
static void check_unmarshal_succeed(const ebpf_inst& ins, const ebpf_platform_t& platform = g_ebpf_platform_linux) {
    ...
    const auto parsed =
        std::get<InstructionSeq>(unmarshal(raw_program{"", "", 0, "", {ins, EXIT_INST, EXIT_INST}, info}));
...
static void check_unmarshal_succeed(ebpf_inst inst1, ebpf_inst inst2,
    ...
    const program_info info{.platform = &platform, .type = platform.get_program_type("unspec", "unspec")};
    const auto parsed =
        std::get<InstructionSeq>(unmarshal(raw_program{"", "", 0, "", {inst1, inst2, EXIT_INST, EXIT_INST}, info}));
    REQUIRE(parsed.size() == 3);
}
...
static void compare_unmarshal_marshal(const ebpf_inst& ins, const ebpf_inst& expected_result,
                                      const ebpf_platform_t& platform = g_ebpf_platform_linux) {
    const program_info info{.platform = &platform, .type = platform.get_program_type("unspec", "unspec")};
    auto parsed = std::get<InstructionSeq>(unmarshal(raw_program{"", "", 0, "", {ins, EXIT_INST, EXIT_INST}, info}));
    REQUIRE(parsed.size() == 3);
...
static void compare_unmarshal_marshal(const ebpf_inst& ins1, const ebpf_inst& ins2, const ebpf_inst& expected_result) {
    const program_info info{.platform = &g_ebpf_platform_linux,
                            .type = g_ebpf_platform_linux.get_program_type("unspec", "unspec")};
    auto parsed =
        std::get<InstructionSeq>(unmarshal(raw_program{"", "", 0, "", {ins1, ins2, EXIT_INST, EXIT_INST}, info}));
    REQUIRE(parsed.size() == 3);
    auto [_, single, _2] = parsed.front();
    (void)_;  // unused
    (void)_2; // unused
...
static void compare_marshal_unmarshal(const Instruction& ins, bool double_cmd = false,
                                      const ebpf_platform_t& platform = g_ebpf_platform_linux) {
    const program_info info{.platform = &platform, .type = platform.get_program_type("unspec", "unspec")};
    auto parsed = std::get<InstructionSeq>(unmarshal(raw_program{"", "", 0, "", marshal(ins, 0), info}));
...
static void check_marshal_unmarshal_fail(const Instruction& ins, std::string expected_error_message,
                                         const ebpf_platform_t& platform = g_ebpf_platform_linux) {
    const program_info info{.platform = &platform, .type = platform.get_program_type("unspec", "unspec")};
    auto actual_error_message = std::get<std::string>(unmarshal(raw_program{"", "", 0, "", marshal(ins, 0), info}));
    boost::algorithm::trim_right(expected_error_message);
    boost::algorithm::trim_right(actual_error_message);
...
static void check_unmarshal_fail(ebpf_inst inst, std::string expected_error_message,
    ...
    auto actual_error_message = std::get<std::string>(result);
    boost::algorithm::trim_right(expected_error_message);
    boost::algorithm::trim_right(actual_error_message);
    REQUIRE(actual_error_message == expected_error_message);
}
...
static void check_unmarshal_fail_goto(ebpf_inst inst, std::string expected_error_message,
                                      const ebpf_platform_t& platform = g_ebpf_platform_linux) {
    const program_info info{.platform = &platform, .type = platform.get_program_type("unspec", "unspec")};
    std::vector insns{inst, EXIT_INST, EXIT_INST};
    auto result = unmarshal(raw_program{"", "", 0, "", insns, info});
    REQUIRE(std::holds_alternative<std::string>(result));
    auto actual_error_message = std::get<std::string>(result);
    boost::algorithm::trim_right(expected_error_message);
...
TEST_CASE("disasm_marshal", "[disasm][marshal]") {
    ...
    SECTION("Call") {
        ...
        check_unmarshal_fail(ebpf_inst{.opcode = INST_OP_CALLX},
                             make_opcode_message("0: bad instruction", INST_OP_CALLX));
        ...
        check_unmarshal_fail({.opcode = INST_OP_CALLX, .dst = 11}, "0: bad register", platform);
        check_unmarshal_fail({.opcode = INST_OP_CALLX, .dst = 8, .imm = 8}, "0: nonzero imm for op 0x8d", platform);
        ...
        check_unmarshal_fail({.opcode = INST_OP_CALLX, .imm = 11}, "0: bad register", platform);
        check_unmarshal_fail({.opcode = INST_OP_CALLX, .imm = -1}, "0: bad register", platform);
    }
...
static void check_instruction_dst_variations(const ebpf_instruction_template_t& previous_template,
    ...
    if (inst.dst == DST) {
        ...
        check_unmarshal_instruction_fail(inst, "0: bad register", platform);
    } else {
        ...
        if (!next_template || !matches_template_inst(inst, next_template->inst)) {
            std::string message;
            if (inst.dst == 1) {
                message = make_opcode_message("0: nonzero dst for register", inst.opcode);
            } else {
                message = make_opcode_message("0: bad instruction", inst.opcode);
            }
            check_unmarshal_instruction_fail(inst, message, platform);
        }
    }
}
...
static void check_instruction_src_variations(const ebpf_instruction_template_t& previous_template,
    ...
    if (inst.src == SRC) {
        ...
        inst.src++;
        if (!next_template || !matches_template_inst(inst, next_template->inst)) {
            check_unmarshal_instruction_fail(inst, make_opcode_message("0: bad instruction", inst.opcode), platform);
        }
    }
}
...
static void check_instruction_offset_variations(const ebpf_instruction_template_t& previous_template,
    ...
    if (inst.offset == JMP_OFFSET) {
        ...
        check_unmarshal_instruction_fail(inst, "0: jump out of bounds", platform);
        ...
        if (!next_template || !matches_template_inst(inst, next_template->inst)) {
            ...
            if (inst.offset == 1 &&
                ...
                messsage = make_opcode_message("0: nonzero offset for", inst.opcode);
            } else {
                messsage = make_opcode_message("0: invalid offset for", inst.opcode);
            }
            check_unmarshal_instruction_fail(inst, messsage, platform);
        }
    }
}
...
static void check_instruction_variations(std::optional<const ebpf_instruction_template_t> previous_template,
    ...
    const uint8_t next_opcode = next_template ? next_template->inst.opcode : 0xFF;
    for (uint8_t opcode = previous_opcode + 1; opcode < next_opcode; opcode++) {
        check_unmarshal_fail({.opcode = opcode}, make_opcode_message("0: bad instruction", opcode));
    }
}
...
TEST_CASE("check unmarshal conformance groups", "[disasm][marshal]") {
    for (const auto& current : instruction_template) {
        ...
        check_unmarshal_fail(current.inst, make_opcode_message("0: bad instruction", current.inst.opcode), platform);
...
TEST_CASE("check unmarshal legacy opcodes", "[disasm][marshal]") {
    ...
    for (const uint8_t opcode : supported_legacy_opcodes) {
        check_unmarshal_fail(ebpf_inst{.opcode = opcode}, make_opcode_message("0: bad instruction", opcode), platform);
    }
}
